...
def dump_autogen_code(fpath, autogen_text, codetype='python', fullprint=None,
    ...
    logger.info('[autogen] Autogenerated %s...\n+---\n' % (fpath,))
    if not dowrite:
        if fullprint:
            ut.print_code(autogen_text, lexer_name=codetype)
            logger.info('\nL___')
        else:
            logger.info('specify --print to write to stdout')
            pass
        logger.info('specify -w to write, or --diff to compare')
        logger.info('...would write to: %s' % fpath)
    if show_diff:
        ...
        if dowrite:
            logger.info('WARNING: Not writing. Remove --diff from command line')
    elif dowrite:
        ut.write_to(fpath, autogen_text)


def makeinit(mod_dpath, exclude_modnames=[], use_star=False):
    r"""Generate a conventional static package initializer.

    Generated initializers are explicit and eager. The historical runtime
    dynamic-import/reload scaffold made package import behavior difficult to
    reason about and provided no lazy-loading benefit. Star-export generation
    is intentionally retired because reconstructing a package's public API by
    introspection can silently change re-exports.

    Args:
        mod_dpath (str): package directory
        exclude_modnames (list): modules to omit
        use_star (bool): unsupported legacy star-export mode
    ...
    if use_star:
        raise ValueError(
            'use_star=True is no longer supported; maintain explicit package '
    ...
    regen_block = '\n'.join([
        '"""',
        'Regen Command:',
        '    {regen_command}'.format(regen_command=regen_command),
        '"""',
    ])
...
def load_func_from_module(modname, funcname, verbose=True, moddir=None, modpath=None):
    ...
    module = None
    error_str = None
    logger.info('modname = %r' % (modname,))
    logger.info('funcname = %r' % (funcname,))
    logger.info('moddir = %r' % (moddir,))

    try:
        from xdoctest import static_analysis as static
        from xdoctest import utils
        logger.info('modpath = {!r}'.format(modpath))
    except ImportError:
        modpath = None
    else:
        if modpath is None:
        ...
        logger.info('module = {!r}'.format(module))
        try:
            func = eval('module.{}'.format(funcname))
        except AttributeError:
            importlib.reload(module)
            func = eval('module.{}'.format(funcname))
        logger.info('func = {!r}'.format(func))
...
def make_default_module_maintest(modname, modpath=None, test_code=None):
    ...
    logger.info('test_code = {!r}'.format(test_code))
